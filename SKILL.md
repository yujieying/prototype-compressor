...
2. Run the bundled script with the source paths:
   ...
   Resolve `scripts/compress_prototype.py` relative to this SKILL.md, or run from the skill directory with an absolute input path. Python 3.10+ and `cwebp` on PATH are required for the default image conversion; `--no-webp` needs only Python.
...
- The script removes ordinary HTML/CSS comments and redundant formatting whitespace.
- It preserves scripts, `<pre>`, `<textarea>`, quoted attribute values, and text-node spacing.
